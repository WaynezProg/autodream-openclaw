...
import {
  detectSupersessionProposals,
  reduceSupersessionChains,
} from "../src/analysis/supersession-detector.js";
...
describe("detectSupersessionProposals", () => {
  ...
  it("does not mark a single weak signal as high confidence", () => {
    const proposals = detectSupersessionProposals([
      makeMem("old", "alpha", {
        metadata: JSON.stringify({ canonical_key: "workflow:isolated-topic" }),
      }),
      makeMem("new", "改用 beta", {
        timestamp: 2,
        metadata: JSON.stringify({ canonical_key: "workflow:isolated-topic" }),
    ...
    expect(proposals).toHaveLength(1);
    expect(proposals[0].confidence).not.toBe("high");
  });

  it("reduces pairwise history to one current node", () => {
    const memories = [
      makeMem("v1", "session cleanup 使用 A", {
        timestamp: 1,
        metadata: JSON.stringify({ canonical_key: "workflow:session-cleanup" }),
      }),
      makeMem("v2", "session cleanup 改用 B，不再使用 A", {
        timestamp: 2,
        metadata: JSON.stringify({ canonical_key: "workflow:session-cleanup" }),
      }),
      makeMem("v3", "session cleanup 改用 C，不再使用 B", {
        timestamp: 3,
        metadata: JSON.stringify({ canonical_key: "workflow:session-cleanup" }),
      }),
    ];

    const reduced = reduceSupersessionChains(detectSupersessionProposals(memories));

    expect(reduced.map((proposal) => proposal.old.id).sort()).toEqual(["v1", "v2"]);
    expect(reduced.every((proposal) => proposal.current.id === "v3")).toBe(true);
  });
});
